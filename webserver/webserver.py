...
def display_movie_award(movie_id):
    cursor = g.conn.execute("SELECT award.name,award.title from movie,won,award WHERE award.award_id=won.award_id AND movie.movie_id=won.movie_id AND movie.movie_id= %s", movie_id)
    result=[]
    for n in cursor:
        result.append([n['name'],n['title']])  # can also be accessed using result[0]
    cursor.close()
    cursor = g.conn.execute("SELECT movie_name from movie WHERE movie_id= %s", movie_id)
    movie_name=cursor.fetchone()['movie_name']
    context= dict(items= result)
    context['movie_id']= movie_id
    context['movie_name']= movie_name
    return render_template("movie_award.html", **context)
...
def get_comment():
    ...
    redirectlink = '/movieid/'+ str(movie_id)+'/comment' 
    return redirect(redirectlink)
...
def add_comment(movie_id):
  ...
  cursor.close()
  link='/movieid/'+str(movie_id)+'/comment'
...
@app.route('/login')
def login():
  mes='Please input your name and account'
  context=dict(mes=mes)
...
@app.route('/login/add_user', methods=['POST'])
def add_user():
  name_user = request.form['name']
  account_user=request.form['account']
  print name_user, account_user
  g.conn.execute("INSERT INTO user_imdb(account, name) VALUES(%s,%s)",account_user,name_user);
