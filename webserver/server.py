...
tmpl_dir=os.path.join('templates')
...
def get_score():
  ...
  print name
  g.conn.execute("")
  score = g.conn.execute("SELECTE AVG(rate_score) FROM feedback,movie WHERE feedback.movie_id=movie.movie_id and movie.movie_name=%s",name)
  return redirect("/")
...
def get_comment():
  ...
  print name
  g.conn.execute("")
  cursor = g.conn.execute("SELECT * FROM feedback JOIN movie ON movie_id WHERE movie.movie_name=%s",name)
  comment = []
  for result in cursor:
    cursor.append(result['reviews'])  # can also be accessed using result[0]
  cursor.close()
  return redirect("/")
@app.route('/director')
def director():
